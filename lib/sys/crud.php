...
class Crud_Service {
    ...
    {
        if(!is_array($data) && is_bool($type)==true) exit(false);
        $this->_dataset = $data;
        $this->_type = $type;
    }
    ...
    public function _DefineDB_Process(){
        ...
        switch($type_query){
            ...
            break;
            case 'create':
                $dbReq = $this->_CreateDB();
            break;
            case 'update':
                $dbReq = $this->_UpdateDB();
            break;
            case 'delete':
                $dbReq = $this->_DeleteDB();
            break;
            case 'showcol':
                $dbReq = $this->_ShowColumn();
            break;
            default:
                $dbReq = false;
            break;
        ...
        return $this->_DBQuery($dbReq);
    }


    protected function _DBQuery($db)
    {
        try
        {
            return (new DB_Class)->connect()->query($db);
        }
